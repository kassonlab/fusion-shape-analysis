...
public:
  ...
  std::list<int>
  count_solid_components(const NT& alpha) const;
...
private:
  ...
  void traverse(Cell_handle pCell,
		...
		const NT alpha) const;
  int traverse_component(Cell_handle pCell,
                         Marked_cell_set& marked_cell_set,
                         const NT alpha) const;
  //----------------------------------------------------------------------
				 ...
				 const NT alpha) const
{
  ...
  template <class Dt>
  std::list<int> Alpha_shape_3<Dt>::count_solid_components(const NT& alpha) const
  {
    typedef typename Marked_cell_set::Data Data;
    Marked_cell_set marked_cell_set(false);
    Finite_cells_iterator cell_it, done = finite_cells_end();
    size_type nb_solid_components = 0;
    std::list<int> component_size;
    ...
    for( cell_it = finite_cells_begin(); cell_it != done; ++cell_it)
    {
      ...
      if (classify(pCell, alpha) == INTERIOR) {
        ...
        if(data == false) {
          // we traverse only interior simplices
          data = true;
          component_size.push_back(traverse_component(pCell, marked_cell_set,
                                                      alpha));
          nb_solid_components++;
        }
      }
  ...
  template <class Dt>
  int Alpha_shape_3<Dt>::traverse_component(Cell_handle pCell,
                                            Marked_cell_set& marked_cell_set,
                                            NT alpha) const
  {
    // traverse neighbors.  Count
    // Based on CGAL alpha_shape_3::traverse
    typedef typename Marked_cell_set::Data Data;
    std::list<Cell_handle> cells;
    cells.push_back(pCell);
    Cell_handle pNeighbor;
